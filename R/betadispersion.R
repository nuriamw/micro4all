...
#'@param formula Model formula to be passed to \code{group} argument from
...
#'@param permutest.options Further arguments to be passed to
#'  \link[vegan]{permutest}. They should be included as a list (see
#'  \code{examples})
#'@param betadisper.options Further arguments to be passed to
...
Betadispersion <- function(data,formula, distances,permutest.options, betadisper.options, type="Samples", ...){
  ...
  for (i in 1:length(distances)){
    ...
    else { res <- do.call(vegan::betadisper, c(list(d=d, group=df[,formula]),betadisper.options))
    res.disper[[i]]=do.call(vegan::permutest, c(list(x=res),permutest.options))
